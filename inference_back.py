...
MODEL_PATH = os.path.join(BASE_DIR, "back_translation_model.keras")
...
def main():
    parser = argparse.ArgumentParser(description="Oromo-to-Amharic inference")
    parser.add_argument("--translate", type=str, help="Translate a single Oromo sentence")
    parser.add_argument("--reference", type=str, help="Reference Amharic sentence for BLEU scoring")
    args = parser.parse_args()

    if args.translate:
        translation = translate(args.translate)
        print("Oromo:", args.translate)
        print("Amharic:", translation)
        if args.reference:
