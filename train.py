...
# FIX: Align the slice shapes to match the input width dimensions explicitly
...
def translate(sentence):
    ...
    for _ in range(dec_seq_len):
        decoder_input = np.zeros((1, dec_seq_len), dtype=np.int32)
        for j in range(len(output)):
            if j < dec_seq_len:
                decoder_input[0, j] = output[j]
